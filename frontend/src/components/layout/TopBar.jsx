...
const TopBar = ({ title, subtitle, candidateName }) => {
  ...
  const { setMobileOpen } = useOutletContext() || { setMobileOpen: () => {} }
  ...
  return (
    <>
      <style>{`
        .topbar { position:sticky; top:0; z-index:40; padding:14px 0; margin-bottom:12px; transition:all 0.3s; }
        @media (min-width: 768px) { .topbar { margin-bottom: 24px; } }
        ...
        @media (min-width: 320px) { .title-area { display: block; } }

        @media (max-width: 767px) {
          .title-area h1 { font-size: 15px !important; }
          .title-area p { font-size: 11px !important; }
        }
      ...
      <div className={`topbar${scrolled ? " scrolled" : ""}`}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button className="tb-icon hamburger" onClick={() => setMobileOpen(true)}>
            ...
            {/* Title Area */}
            <div className="animate-fade-up title-area">
              {scrolled && candidateName ? (
                ...
                <>
                  <h1 style={{ fontSize:18, fontWeight:700, color:darkMode?"#f9fafb":"#0f172a", fontFamily:"Satoshi,Inter,sans-serif", letterSpacing:"-0.3px", lineHeight:1.2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "200px" }}>
                    {title}
                  </h1>
                  {subtitle && <p style={{ fontSize:12, color:darkMode?"#94a3b8":"#64748b", marginTop:2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "240px" }}>{subtitle}</p>}
                </>
              )}
            </div>
          </div>
            ...
            {!isMobile ? (
              <>
                ...
                <div style={{ position:"relative" }} ref={notifRef}>
                  ...
                  {notifOpen && (
                    <div className="dropdown" style={{ minWidth:260, right: 0 }}>
              ...
              <div style={{ position:"relative" }} ref={moreRef}>
                <button className="tb-icon" onClick={() => setMoreOpen(!moreOpen)}>
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="5" r="2" fill="currentColor"/><circle cx="12" cy="12" r="2" fill="currentColor"/><circle cx="12" cy="19" r="2" fill="currentColor"/></svg>
                </button>
                {moreOpen && (
                  <div className="dropdown" style={{ right: 0, minWidth: 180 }}>
                    <div style={{ padding: "8px 12px", borderBottom: `1px solid ${darkMode ? "#1f2937" : "#f1f5f9"}` }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: darkMode ? "#f9fafb" : "#0f172a" }}>Account</p>
                    </div>
                    <button className="dd-item" onClick={() => { toggleDarkMode(); setMoreOpen(false); }}>
                      {darkMode ? "☀️ Light Mode" : "🌙 Dark Mode"}
                    </button>
                    <button className="dd-item" onClick={() => { navigate("/dashboard/settings"); setMoreOpen(false); }}>
                      ⚙️ Settings
                    </button>
                    <button className="dd-item" onClick={() => { setNotifOpen(true); setMoreOpen(false); }}>
                      🔔 Notifications
                    </button>
                    <div className="dd-sep"/>
                    <button className="dd-item danger" onClick={handleLogout}>
